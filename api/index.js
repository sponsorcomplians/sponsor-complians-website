...
var emailCampaignRouter = router({
  ...
  campaigns: router({
    ...
    generateEmailContent: adminProcedure.input(z3.object({
      prompt: z3.string().min(1),
      tone: z3.enum(["professional", "friendly", "urgent", "educational", "promotional"]).optional(),
      campaignName: z3.string().optional()
    })).mutation(async ({ input }) => {
      try {
        const toneGuide = {
...
Tone: ${input.tone}` : ""}`
        ...
        blocks = blocks.filter((b) => b && b.type && validTypes.includes(b.type) && b.content);
        return { blocks };
      } catch (err) {
